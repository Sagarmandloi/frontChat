...
const Form = ({
    ...
    const handleSubmit = async(e) => {
        ...
        e.preventDefault()
        const res = await fetch(`https://mychatapp-back-1.onrender.com/api/${isSignInPage ? 'login' : 'register'}`, {
            method: 'POST',
            headers: {
  ...
  return (
    <div className="bg-light h-screen flex items-center justify-center">
        <div className=" bg-white w-[600px] h-[800px] shadow-lg rounded-lg flex flex-col justify-center items-center">
            ...
            { !isSignInPage && <Input label="Full name" name="name" placeholder="Enter you email" className="mb-6 w-[75%]" value={data.fullName} onChange={(e) => setData({ ...data, fullName: e.target.value }) } /> }
